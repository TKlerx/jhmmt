...
public class KMeansLearner<O extends Observation & CentroidFactory<? super O>> {
	 ...
	 * 
	 * @param nbStates
	 *            The number of states the resulting HMM will be made of.
	 * @param opdfFactory
	 *            A class that builds the observation probability distributions
	 *            associated to the states of the HMM.
	 * @param sequences
	 *            A vector of observation sequences. Each observation sequences
	 *            is a vector of
	 *            {@link be.ac.ulg.montefiore.run.jahmm.Observation
	 *            observations} compatible with the
	 *            {@link be.ac.ulg.montefiore.run.jahmm.CentroidFactory k-means
	 *            algorithm}.
	 */
	...
	private void learnPi(Hmm<?> hmm) {
		...
		for (int i = 0; i < nbStates; i++) {
			double temp = pi[i] / obsSeqs.size();
			if(Double.isNaN(temp)){
				System.err.println("temp is NaN");
			}
			hmm.setPi(i, temp);
		}
	}
	...
	private void learnAij(Hmm<O> hmm) {
		...
		for (List<? extends O> obsSeq : obsSeqs) {
			...
			for (int i = 1; i < obsSeq.size(); i++) {
				...
				second_state = clusters.clusterNb(obsSeq.get(i));

				hmm.setAij(first_state, second_state, hmm.getAij(first_state, second_state) + 1.);
			}
		}
