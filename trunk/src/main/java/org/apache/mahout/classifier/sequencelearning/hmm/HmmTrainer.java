...
public final class HmmTrainer {
	 ...
	 * 
	 * @param nrOfHiddenStates
	 *            The total number of hidden states
	 * @param nrOfOutputStates
	 *            The total number of output states
	 * @param observedSequence
	 *            Integer array containing the observed sequence
	 * @param hiddenSequence
	 *            Integer array containing the hidden sequence
	 * @param pseudoCount
	 *            Value that is assigned to non-occurring transitions to avoid
	 *            zero probabilities.
	...
	public static HmmModel trainSupervised(int nrOfHiddenStates, int nrOfOutputStates, int[] observedSequence, int[] hiddenSequence, double pseudoCount) {
		...
		countTransitions(transitionMatrix, emissionMatrix, observedSequence, hiddenSequence);
	 ...
	 *            transition matrix to use.
	 * @param emissionMatrix
	 *            emission matrix to use for counting.
	 * @param observedSequence
	 *            observation sequence to use.
	 * @param hiddenSequence
	...
	private static void countTransitions(Matrix transitionMatrix, Matrix emissionMatrix, int[] observedSequence, int[] hiddenSequence) {
		emissionMatrix.setQuick(hiddenSequence[0], observedSequence[0], emissionMatrix.getQuick(hiddenSequence[0], observedSequence[0]) + 1);
		for (int i = 1; i < observedSequence.length; ++i) {
			transitionMatrix.setQuick(hiddenSequence[i - 1], hiddenSequence[i], transitionMatrix.getQuick(hiddenSequence[i - 1], hiddenSequence[i]) + 1);
			emissionMatrix.setQuick(hiddenSequence[i], observedSequence[i], emissionMatrix.getQuick(hiddenSequence[i], observedSequence[i]) + 1);
		}
	}

	/**
	 * Create an supervised initial estimate of an HMM Model based on a number
	 * of sequences of observed and hidden states.
	 * 
	 * @param nrOfHiddenStates
	 *            The total number of hidden states
	 * @param nrOfOutputStates
	 ...
	 *            Collection of hidden sequences to use for training
	 * @param observedSequences
	 *            Collection of observed sequences to use for training
	 *            associated with hidden sequences.
	 * @param pseudoCount
	 *            Value that is assigned to non-occurring transitions to avoid
	 *            zero probabilities.
	 * @return An initial model using the estimated parameters
	 */
	public static HmmModel trainSupervisedSequence(int nrOfHiddenStates, int nrOfOutputStates, Collection<int[]> hiddenSequences,
		...
		while (hiddenSequenceIt.hasNext() && observedSequenceIt.hasNext()) {
			...
			initialProbabilities.setQuick(hiddenSequence[0], initialProbabilities.getQuick(hiddenSequence[0]) + 1);
			countTransitions(transitionMatrix, emissionMatrix, observedSequence, hiddenSequence);
		}
	 ...
	 *            expensive but offers better numerical stability for large
	 *            observed sequences
	...
	public static HmmModel trainViterbi(HmmModel initialModel, int[] observedSequence, double pseudoCount, double epsilon, int maxIterations, boolean scaled) {
		...
		for (int i = 0; i < maxIterations; ++i) {
			...
			countTransitions(transitionMatrix, emissionMatrix, observedSequence, viterbiPath);
	 ...
	 *            The maximum number of training iterations
	 * @param scaled
	 *            Use log-scaled implementations of forward/backward algorithm.
	 *            This is computationally more expensive, but offers better
	 *            numerical stability for long output sequences.
	 * @return The iterated model
	 */
	...
	private static void logScaledBaumWelch(int[] observedSequence, HmmModel iteration, Matrix alpha, Matrix beta) {
		...
		for (int i = 0; i < iteration.getNrOfHiddenStates(); ++i) {
			for (int j = 0; j < iteration.getNrOfHiddenStates(); ++j) {
				...
				for (int t = 0; t < observedSequence.length - 1; ++t) {
					double temp = alpha.getQuick(t, i) + Math.log(emissionMatrix.getQuick(j, observedSequence[t + 1])) + beta.getQuick(t + 1, j);
	...
	/**
	 * Check convergence of two HMM models by computing a simple distance
	 * between emission / transition matrices
	 * 
	 * @param oldModel
	 *            Old HMM Model
	 * @param newModel
